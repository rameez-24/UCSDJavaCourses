...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		String currWord = "";
		while(!queue.isEmpty() && !currWord.equals(word2)) {
			WPTreeNode currNode = queue.remove(0);			
			currWord = currNode.getWord();
			List<String> allChildren = nw.distanceOne(currWord, true);
			for (String w : allChildren) {
				if (!visited.contains(w)) {
					WPTreeNode newNode = currNode.addChild(w);
					visited.add(w);
					queue.add(newNode);
					if (w.equals(word2)) {
...
//			System.out.println(printQueue(currNode.getChildren()));
	...
	public static void main(String[] args) {
		...
		System.out.println(wp.findPath("tail","kangarooo"));
	}
